...
export function formalDecision(verdict: Verdict, reasons: string[], metrics: ExperimentMetric[], economicsComplete: boolean): FormalDecision {
  ...
  const actions: Record<FormalDecision["decision"], string> = {
    CONTINUE_TESTING: "Continue the bounded test and monitor the weakest funnel signal.", SCALE: "Increase spend gradually while monitoring contribution after advertising.", REWORK_OFFER: "Revise price, bundle, or offer before the next test.", CHANGE_CREATIVE: "Test a new creative hypothesis against the same offer.", CHANGE_AUDIENCE: "Test a new audience while holding the offer and creative constant.", CHANGE_LANDING: "Revise the landing experience while holding traffic constant.", CHANGE_SUPPLIER: "Verify a lower-risk source with viable landed cost.", KILL: "Stop allocating test capital and preserve the learning.", INSUFFICIENT_DATA: "Wait for more measured data.", VERIFY_DATA: "Complete and verify the missing inputs.", WAIT_FOR_MORE_DATA: "Continue measuring until the decision threshold is reached.",
  };
  return {
    decision,
    why: reasons,
